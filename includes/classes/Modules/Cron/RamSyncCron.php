...
use wp_coremanager\Modules\RamSync;

class RamSyncCron extends Module
{
    ...
    public function registrar()
    {
        ...
        if (! wp_next_scheduled('wp_coremanager_sync_ram_event')) {
            wp_schedule_event(time(), 'twicedaily', 'wp_coremanager_sync_ram_event');
        }
        // Asocia el evento a la función de sincronización
        add_action('wp_coremanager_sync_ram_event', [$this, 'trigger_sync']);
    }
    ...
    {
        $timestap = wp_next_scheduled('wp_coremanager_sync_ram_event');
        if ($timestap) {
            wp_unschedule_event($timestap, 'wp_coremanager_sync_ram_event');
        }
    }
}
